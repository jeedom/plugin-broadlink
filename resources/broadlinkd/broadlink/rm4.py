...
def read_rm4(device):
	...
	mac = device['mac']
	name = device['name']
	product = broadlink.rm4(host=(host,int(port)), mac=bytearray.fromhex(mac))
	logging.debug("Connecting to Broadlink RM4 device with name " + name + "....")
	product.auth()
	logging.debug("Connected to Broadlink RM4 device with name " + name + "....")
	result['mac']=mac
